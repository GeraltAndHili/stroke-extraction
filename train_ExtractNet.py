...
class TrainExtractNet():
    ...
    def __consume_pool_batch(self, batch_num, optim_opWhole=None):
        target_data_batch, reference_stroke_transformation_data_batch, segment_data_batch, \
        reference_segment_transformation_data_batch, segNet_feature_batch, label_batch, cut_box_list_batch = self.data_pool.next(batch_num)
        ...
        if optim_opWhole is not None:
            loss.backward()
            optim_opWhole.step()
            optim_opWhole.zero_grad()

        torch.cuda.empty_cache()
        return [loss.item(), mIOUm.item(), 0]
    ...
    def train_model(self, epochs=40, batch_size=16, init_learning_rate=0.001, dataset=None, val_ratio=0.1, seed=42):
        ...
        for i in range(epochs):
            ...
            val_loss, loss_name = self.__eval_epoch(i, val_loader, stage_label='VAL')
    ...
    def evaluate_model(self, dataset=None, split='test'):
        eval_loader = data.DataLoader(
            SegNetExtractNetLoader(is_training=False, dataset_path=dataset, is_single=True, split=split),
            batch_size=1,
        )
        return self.__eval_epoch(0, eval_loader, stage_label=split.upper())
    ...
    def __train_epoch(self, epoch, train_loader, optim_opWhole):
        ...
        for i, batch_sample in enumerate(train_loader):
            ...
            while self.data_pool.data_num >= self.batch_size:
                loss_list.append(self.__consume_pool_batch(self.batch_size, optim_opWhole))

        if self.data_pool.data_num > 0:
            loss_list.append(self.__consume_pool_batch(self.data_pool.data_num, optim_opWhole))

        loss_name = ['loss',  'mIOUm', 'mIOUum']
        if not loss_list:
            loss_value = np.zeros(len(loss_name), dtype=np.float32)
            print('[TRAIN] No batches were processed.')
    ...
    def __eval_epoch(self, epoch, data_loader, stage_label='VAL'):
        epoch += 1
        ...
        if not loss_list:
            loss_value = np.zeros(len(loss_name), dtype=np.float32)
            print(f'[{stage_label}] No batches were processed.')
            return loss_value, loss_name
        loss_value = np.mean(np.array(loss_list), axis=0)

        print(
            "[{}][{}/{}], loss={:.7f},  mIOUm={:.7f}, mIOUum={:.7f}, time={:.7f}".format(
                stage_label, i, len(data_loader), loss_value[0], loss_value[1], loss_value[2], time.time() - start_time))
        return loss_value, loss_name
