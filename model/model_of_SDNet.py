...
import numpy as np
char_model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), r'../char_recognise/out_vgg_bn/model')
...
class SDNet(nn.Module):
    ...
    def __init__(self, image_size=(256, 256)):
        ...
        coordinate = torch.stack(coordinate)[[1, 0]].float()  # y, x 璋冩暣涓簒,y
        self.coordinate = coordinate.numpy()

    def get_two_registration_field(self, color_reference, target_data):
        '''
        Calculate Registration field of 桅_d and 桅_s
        '''
        x_whole, x_linear = self.unet_model(color_reference, target_data)
        flow_global = self.flow_whole(x_whole) * 10   # Registration field of 桅_d
        flow_linear_ = self.flow_linear(x_linear)*5
        flow_linear = flow_global + flow_linear_  # Registration field of 桅_s
    ...
    def __get_inverse_grid(self, grid):
        ...
        x = np.array([grid_[0, :, 0, 0], grid_[0, :, 0, 255], grid_[0, :, 255, 0]], dtype=np.float64)
        y = np.array([[0, 0], [255, 0], [0, 255]], dtype=np.float64)
        expand = np.ones(shape=(3, 1), dtype=np.float64)
        x = np.concatenate([x, expand], axis=1)
        map = np.dot(np.linalg.inv(x), y)
        grid_base = self.coordinate.reshape((2, -1)).transpose((1, 0)).astype(np.float64)
        grid_base = np.concatenate([grid_base, np.ones(shape=(256*256, 1), dtype=np.float64)], axis=1)
        d_grid = np.round(np.dot(grid_base, map)).astype(np.int64)
        d_grid = np.clip(d_grid[:, ], 0, 255).reshape((1, 256, 256, 2)).transpose((0, 3, 1, 2 )).astype(np.float64)
    ...
    def get_linear_estimation(self, reference_single_stroke, grids, reference_single_stroke_centroid, inverse=False):
        ...
        mark = (reference_single_stroke > 0.5).float().cuda()  # 锛圢, 1, 256, 256)
...
class UNetWithFeature(nn.Module):
    ...
    def __init__(self,  with_char_feature=True):
        ...
        if with_char_feature:
            ...
            char_model_path = self._find_char_model_path()
            if char_model_path is not None:
                state = torch.load(char_model_path)
                self.char_net.load_state_dict(state['state'])
                print(f"Successful to load parameters of CharNet in SDNet from {char_model_path}!")
            else:
                print("Failed to load parameters of CharNet in SDNet!")
            self.char_net.eval()  # 鍥哄畾BN鍙傛暟
            self.char_net.requires_grad_(False)  # 鍋滄鏇存柊鍙傛暟

    @staticmethod
    def _find_char_model_path():
        for file_name in ('model.pth', 'model.th'):
            candidate = os.path.join(char_model_dir, file_name)
    ...
    def forward(self, color_reference, target_image):
        ...
        # 8
        x = self.block3(x)  # 8
        x = torch.cat([x, char_feature_reference['out_8_128'], char_feature_target['out_8_128']], dim=1)  # 娣诲姞璇嗗埆鐗瑰緛
